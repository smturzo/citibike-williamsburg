...
Usage: python3 collect/collect.py [--src mac|gha]
"""
...
def main():
    ...
    raw_path = os.path.join(ROOT, "data", "raw", day, f"{hhmm}-{args.src}.csv")
    ...
    try:
        ...
        append_rows(cov_path, COV_FIELDS, [{
            "ts_bucket": bucket, "src": args.src, "ok": 0, "n_stations": 0,
    ...
    append_rows(cov_path, COV_FIELDS, [{
        "ts_bucket": bucket, "src": args.src, "ok": 1, "n_stations": len(rows),
    ...
    print(f"{stamp} src={args.src} stations={len(rows)}/{len(tracked)} "
