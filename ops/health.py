...
def main():
    ...
    rows = con.execute("""
        SELECT local_date,
               COUNT(DISTINCT ts_bucket) AS buckets,
               COUNT(DISTINCT station_id) AS stations
        FROM snapshots GROUP BY local_date ORDER BY local_date DESC LIMIT 21
    """).fetchall()
    if not rows:
        print("  no snapshots yet")
    for r in rows:
        frac = min(r["buckets"] / EXPECTED_PER_DAY, 1.0)
        print(f"  {r['local_date']}  {bar(frac)}  {r['buckets']:3d}/288 "
              f"({100*frac:5.1f}%)  {r['stations']} stations")
    ...
    hours = {r["h"]: r["n"] for r in con.execute(
        "SELECT mod/60 AS h, COUNT(DISTINCT ts_bucket) AS n FROM snapshots GROUP BY h")}
    n_days = con.execute("SELECT COUNT(DISTINCT local_date) FROM snapshots").fetchone()[0] or 1
    for h in range(24):
        got = hours.get(h, 0)
        frac = min(got / (12 * n_days), 1.0)
        flag = "  <-- blind" if frac < 0.5 else ""
        print(f"  {h:02d}:00  {bar(frac)}  {100*frac:5.1f}%{flag}")
    ...
    for r in con.execute("""
        SELECT src, COUNT(*) AS attempts, SUM(ok) AS good,
               AVG(fetch_ms) AS ms
        FROM coverage GROUP BY src"""):
        fails = r["attempts"] - (r["good"] or 0)
        print(f"  {r['src']:4s}  {r['attempts']:6d} attempts  {fails:3d} failed  "
              f"avg {r['ms']:.0f}ms")

    fails = con.execute(
        "SELECT ts_bucket, src, note FROM coverage WHERE ok=0 "
        "ORDER BY ts_bucket DESC LIMIT 5").fetchall()
    if fails:
        print("  recent failures:")
        for r in fails:
            t = datetime.fromtimestamp(r["ts_bucket"], NY).strftime("%m-%d %H:%M")
            print(f"    {t} [{r['src']}] {r['note'][:60]}")

    # Longest stretch with no data at all.
    print()
    buckets = [r[0] for r in con.execute(
        "SELECT DISTINCT ts_bucket FROM snapshots ORDER BY ts_bucket")]
    if len(buckets) > 1:
        gaps = [(b - a, a, b) for a, b in zip(buckets, buckets[1:]) if b - a > 600]
